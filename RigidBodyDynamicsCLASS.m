...
%            standard values.  
%  B.delete()         
%            Removes the body from the graphics output and the  memory   
...
%            coordinates.  The point is provided in components of B.
%  I_v_Q = B.velocityOfPoint(B_r_BQ)   
%            Returns the absolute velocity of a body fixed point Q in
%            inertial coordinates.  The point is provided in components of
%            B. 
...
classdef RigidBodyDynamicsCLASS < handle
    ...
    properties (SetAccess = private, GetAccess = private)
        ...
        v_r;         % Vector of linear displacement
    ...
    properties
        ...
        scale = 1;               % To draw the coordinate systems of the 
                                 % body smaller
    end
    % methods
    methods
        ...
        function obj = RigidBodyDynamicsCLASS(env)
            ...
            obj.v_r               = VectorCLASS(env, obj.B, obj.B_r_IB);
            obj.v_r.color         = [0.7;0.7;0.7];
            ...
            obj.ellipsoidPatch = patch('faces', f, 'vertices', v, 'FaceColor', [0;0;1],'EdgeColor', 'none','FaceAlpha',0.5);
            obj.update();
        end
        ...
        function updateGraphics(obj)
            update(obj)
            % Calling resetOutput, adjusts the scaling of the graphical
            % output window, such that all vectors and coordinate systems
            % are visible.
            obj.env.resetOutput();
            drawnow();
        end
        function computeNaturalDynamics(obj)
            ...
            obj.B_omegaDot_B  = obj.B_I_B \ (B_LDot_B - B_omega_IB*B_p);
        end
	end
    methods (Access = protected)
        ...
        function update(obj)
            ...
            obj.v_r.setCoords(obj.B, obj.B_r_IB);
